...
class Google_Widget_Reviews_Badge extends WP_Widget
{
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            'njt_google_review_widget_badge', // Base ID
            'Google Places Reviews Badge', // Name
            array(
                'classname' => 'njt-google-places-reviews',
                'description' => __('Display user reviews for any location found on Google Places.', 'njt-google-reviews'),
            )
        );
    ...
    public function widget($args, $instance)
    {
        ...
        $response = get_transient('njt_grv_widget_badge_api_' . $transient_unique_id);
        $widget_options = get_transient('njt_grv_widget_badge_options_' . $transient_unique_id);
        ...
        if ($cache !== 'none') {
            if ($response === false || $serialized_instance !== $widget_options) {
                ...
                set_transient('njt_grv_widget_badge_api_' . $transient_unique_id, $response, $expiration);
                set_transient('njt_grv_widget_badge_options_' . $transient_unique_id, $serialized_instance, $expiration);
            }
        }
        ...
        $args_post_reviews = apply_filters('njt_ggrv_get_reviews', $args_post_reviews);
        $postReviews = get_posts($args_post_reviews);
        //var_dump($postReviews);
        echo '<div class="njt-google-places-reviews-wap ' . $disshahow . '">';
        if (count($postReviews) > 0) {
            include NJT_PLUGIN_PATH . '/includes/views/widget-badge.php';
        } else {
    ...
    public function form($instance)
    {
        ...
        if (!isset($this->api_key) || empty($this->api_key)) {
            ...
            include NJT_PLUGIN_PATH . '/includes/views/widget-badge-form.php';
        }
